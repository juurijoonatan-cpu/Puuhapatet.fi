...
import { ArrowLeft, ArrowRight, Loader2, Plus, Building2, AlertTriangle } from "lucide-react";
...
export default function AdminGigsPage() {
  ...
  useEffect(() => {
    if (!isHost) return;
    api.getGigMoney().then((res) => { if (res.ok && res.data) setMoney(res.data); });
  }, [isHost]);
      ...
      <div className="container mx-auto px-4 max-w-3xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-muted-foreground text-sm">
              {loading ? "Ladataan…" : `${sorted.length} keikkaa`}
            </p>
          </div>
        ...
        {!loading && sorted.length > 0 && (
          <div className="space-y-3">
            {sorted.map(({ job, customer }) => {
              const m = moneyByJob.get(job.id);
              /**
               * Nimi. Yrityksen nimi asuu `GigData.company`ssa jota tämä reitti
               * ei tuo (ks. `GigJobRow`), joten paras saatavilla oleva lähde on
               * rahareitin `name` — palvelin muodostaa sen juuri siitä blobista
               * (`gig?.company?.name || job.description`). Ilman rahaa keikkaa
               * ei ole rahareitillä lainkaan, ja silloin jää kuvaus.
               */
              const gigName = m?.name || job.description || `Keikka #${job.id}`;
              // Yhdistyksen/yrityksen nimi on eri asia kuin yhteyshenkilö —
              // molemmat näytetään, jottei "Stuhi" ja "Matti" sekoitu.
               ...
               * (`isCommunityGig`), ja se asuu projektiblobissa jota tämä lista
               * ei lue. Mitä tiedämme: palvelin kirjoittaa yhteisökeikan
               * `agreedPrice`n NIMENOMAISESTI nollaksi joka projektin
               * tallennuksella ("vapaaehtoistyö ei saa päätyä agreedPriceen"),
               * ja maksullinen keikka syntyy `/admin/new-gig`issa aina
               * positiivisella sektorihinnalla. Nolla on siis vahva vihje —
               * mutta se on vihje: sen voi saada myös keikka jonka sektorit on
               * käsin nollattu. Siksi merkki on "0 €", ei väite korvaustavasta,
               * ja varmistus tapahtuu keikan asetuksissa.
               */
              const communityHint = job.agreedPrice === 0;

              return (
                // Linkki on itse osumakohde: <a> antaa näppäimistöfokuksen,
                // Enterin ja "avaa uuteen välilehteen" ilmaiseksi. Pelkkä
                // onClick-div ei anna niistä mitään.
                <Link
                  key={job.id}
                  ...
                  <Card className="p-4 bg-card border-0 premium-shadow hover:opacity-90 transition-opacity">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 mt-3">
                      ...
                      {communityHint ? (
                        ...
                        // `/admin/jobs`-listalla, joten tässä ei ole uutta
                        // tietovuotoa — rahaportti koskee gig-money-lukuja.
                        <Badge variant="secondary" title="Sopimusarvo: kiinteä katto + lukitut keltaiset">
                          Sopimus {fmt(job.agreedPrice)}
                        </Badge>
                      )}

                      {/* Etenemä siinä tarkkuudessa kuin tämä lista sen tietää:
                          keikan status. Ikkunakohtainen prosentti tulisi
                          projektiblobista, jota ei lueta listalle. */}
                    ...
                    {isHost && m && (
                      <div className="grid grid-cols-3 gap-2 mt-3">
                        {[
                          ...
                          {
                            // Sama luku kuin dashboardin "Jää meille":
                            // laskutettu − tekijöiden ANSAINTA (ei maksettu),
                            // eli kate ei näytä paremmalta vain siksi että
                            // palkat ovat vielä maksamatta.
                            label: "Kate",
                    ...
                    {/* Rahareitti jättää rahattoman keikan pois. Nollat
                        näyttäisivät virheeltä ("miksi laskutettu 0 €?"), joten
                        tilalle sanotaan mikä tilanne oikeasti on. */}
                    {isHost && money && !m && (
                    ...
                    {isHost && m && m.unassignedEraCount > 0 && (
                      <p className="mt-2 flex items-start gap-1.5 text-xs text-amber-600 dark:text-amber-400">
                        ...
                        <span>
                          {m.unassignedEraCount} erä{m.unassignedEraCount === 1 ? "" : "ä"} ilman
                          merkintää siitä kumpi rahat sai — kohdentamaton raha ei kuulu kenellekään
                          eikä ole tasauksessa mukana.
                        </span>
                      </p>
                    )}

                    {/* SUUNTA MUKAAN. Pelkkä summa ("Tasaus 1 271,25 €") oli
                        tässä repossa jo kertaalleen virhe: se on ainoa paikka
                        joka tietää kuka maksaa, eikä se kertonut sitä. Nimet
                        ovat roolimerkinnän takana ("maksaja"/"saaja"), koska
                        taivutettu muoto ("Matias Pitkänenlle") menisi väärin. */}
                    {/* Alle euron siirto on pyöristyskohinaa (sama raja kuin
                        dashboardin tasauslauseessa) — ei varoituksen arvoinen. */}
